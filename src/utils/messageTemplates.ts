...
const readAmount = (context: TemplateContext): string => {
...
const readCurrency = (context: TemplateContext): string =>
  readString(context, "currency", "FCFA");

const readReference = (context: TemplateContext): string =>
  readString(context, "transactionId", "N/A");

const readBalance = (context: TemplateContext): string => {
...
const readCode = (context: TemplateContext): string =>
  readString(context, "code", "N/A");

const resolveStatusLabel = (context: TemplateContext): string => {
...
export const generateMessage = (
  ...
  switch (type) {
    ...
    case TypeNotification.PAIEMENT_ECHOUE:
      return `Votre paiement de ${amount} ${currency} a échoué. Référence: ${reference}.`;

    case TypeNotification.FACTURE_GENEREE:
      return `Une nouvelle facture a été générée${
        readString(context, "factureId", "").length > 0
          ? ` (ID: ${readString(context, "factureId", "")})`
          : ""
      }.`;

    case TypeNotification.FACTURE_PAYEE:
      return `Votre facture a été payée avec succès${
        readString(context, "factureId", "").length > 0
          ? ` (ID: ${readString(context, "factureId", "")})`
          : ""
      }.`;

    case TypeNotification.TENTATIVE_FRAUDE:
      return `Une tentative de fraude a été détectée sur votre compte. Nos équipes ont été alertées.`;

    case TypeNotification.TRANSACTION_SUSPECTE:
      return `Une transaction suspecte a été détectée. Montant: ${amount} ${currency}. Référence: ${reference}.`;

    case TypeNotification.ACTIVITE_INHABITUELLE:
    ...
    case TypeNotification.MAINTENANCE:
      return `Une opération de maintenance est programmée${
        readString(context, "window", "").length > 0
          ? `: ${readString(context, "window", "")}`
          : " prochainement"
      }.`;

    case TypeNotification.MISE_A_JOUR_SYSTEME:
